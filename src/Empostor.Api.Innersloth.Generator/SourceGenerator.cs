...
using CSharpPoet;
using Empostor.Api.Innersloth.Generator.Generators;
using Microsoft.CodeAnalysis;

namespace Empostor.Api.Innersloth.Generator;
...
public sealed class SourceGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(filesProvider, (spc, files) =>
        {
            if (files.IsEmpty)
            {
                throw new InvalidOperationException($"No json files found in Empostor.Api/{DataPath}");
            }

            var enumGenerator = new EnumGenerator(spc, files);

            enumGenerator.Generate("ColorType", "Empostor.Api.Innersloth.Customization");
