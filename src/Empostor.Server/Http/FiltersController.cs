...
public sealed class FiltersController : ControllerBase
{
    ...
    public IActionResult GetFilters([FromHeader] AuthenticationHeaderValue authorization)
    {
        ...
        TokenController.TokenResponse? token = null;
        try
        {
            token = JsonSerializer.Deserialize<TokenController.TokenResponse>(Convert.FromBase64String(authorization.Parameter));
        }
        catch
        {
            return BadRequest();
