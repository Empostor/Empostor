...
using Empostor.Server.Commands;
...
namespace Empostor.Server.Net.Inner.Objects
{
    internal partial class InnerPlayerControl : InnerNetObject
    {
        ...
        internal System.Collections.Concurrent.ConcurrentQueue<string> RequestedPlayerName { get; } = new();

        internal System.Collections.Concurrent.ConcurrentQueue<ColorType> RequestedColorId { get; } = new();
        ...
        private async ValueTask<bool> HandleSetName(ClientPlayer sender, string name)
        {
            ...
            {
                if (RequestedPlayerName.TryDequeue(out var expected))
                {
        ...
        private async ValueTask<bool> HandleSetColor(ClientPlayer sender, ColorType color)
        {
            ...
            {
                if (RequestedColorId.TryDequeue(out var requested))
                {
                    if (Game.IsColorUsed(color, this))
                    {
        ...
        private async ValueTask<bool> HandleSendChat(ClientPlayer sender, string message)
        {
            if (message.StartsWith("/"))
            {
                ...
                {
                    Name = message.Length > 1 ? message[1..].Split(' ')[0].ToLowerInvariant() : string.Empty,
                    RawArgs = message.Length > 1 && message.Contains(' ') ? message[(message.IndexOf(' ') + 1)..] : string.Empty,
                    Args = message.Length > 1 && message.Contains(' ')
                        ? message[(message.IndexOf(' ') + 1)..].Split(' ', System.StringSplitOptions.RemoveEmptyEntries)
