...
namespace Empostor.Server.Net.State
{
    internal partial class Game : IGame
    {
        ...
        public async ValueTask SyncSettingsAsync()
        {
            ...
            {
                throw new EmpostorException("Attempted to change settings when the host was not spawned.");
            }

            if (GameNet.GameManager == null)
            {
                throw new EmpostorException("Attempted to change options when the game manager was not spawned.");
            }

            var gameOptionsTag = GameNet.GameManager.GetGameLogicTag(GameNet.GameManager.LogicOptions);
            if (gameOptionsTag == -1)
            {
                throw new EmpostorException("Attempted to change options when the LogicOptions was not spawned.");
            }
